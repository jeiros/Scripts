...
def LabelTrajectories(TrajectorySet, nClusters):
    """Instatiate an object of the KCenters class and call it's fit method on the TrajectorySet obtained through loadTrajs. Returns a labeled trajectory ( A sequence of integers corresponding to the index of the conformational state occupied by the system at each time point on a trajectory. )"""
...
def MakeMSM(LabeledTrajs):
    """Returns a matrix with i columns and j rows. The element i,j of the matrix contains the Transition Matrix of the MSM created with the amount of clustering centers corresponding to the i row and using the lag time corresponding to the j column."""
...
def saveTransMatrices(transmat_matrix):
    """Retrieve each of the i x j Transition Matrices and store each one in different files in the msmResults directory. The files are named Transmat_Nclusters_Nlagtime"""

    for i in range(len(cluster_list)):
        for j in range(len(lag_times)):
            item = transmat_matrix[i][j]
            if os.path.exists("./msmResults"):
                np.savetxt("./msmResults/Transmat_{0}clusters_{1}lagtime.dat".format(str(cluster_list[i]), str(lag_times[j])), item)
            else:
                os.mkdir("./msmResults")
                np.savetxt("./msmResults/Transmat_{0}clusters_{1}lagtime.dat".format(str(cluster_list[i]), str(lag_times[j])), item)
